...
import pytorch_ssim
import math
from model.opticalSimulation import ONN # import your model here
...
def main(args):
    ...
    vol_size = 0.000025  # 印製時最小可操控的高度 (單位 m)
    H_thickness = 0.003  # 最大厚度 (m)
    ...
    model = ONN()  # load your model here
    model.cuda()  # move to cuda
    model.load_state_dict(torch.load(args.model_save_path + args.model_name))  # load your weight # !!!
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model-save-path', type=str, default="./checkpoints_weights/baseline_restormer_ONN/weights")
    parser.add_argument('--model-name', type=str, default='.pth')

    torch.backends.cudnn.benchmark = True  # 讓 cuDNN 自動挑選最快的卷積實作方式
    args_ = parser.parse_args()
    random.seed(args_.seed)  # 設定python seed, 影響random.random()、random.shuffle()
    np.random.seed(args_.seed)  # 設定NumPy seed 
    torch.manual_seed(args_.seed)  # 設定 PyTorch seed
    torch.cuda.manual_seed_all(args_.seed)  # 設定所有GPU seed (多GPU尤其重要)
    main(args_)
